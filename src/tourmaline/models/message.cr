...
module Tourmaline
  ...
  class Message
    ...
    def pin(**kwargs)
      client.pin_message(chat, message_id, **kwargs)
    end
    ...
    def respond(message, **kwargs)
      client.send_message(chat, message, **kwargs, reply_to_message: nil)
    end
    ...
    {% for content_type in %w[audio animation contact document location photo media_group venu video video_note voice invoice poll dice dart basket] %}
      ...
      def respond_with_{{content_type.id}}(*args, **kwargs)
        client.send_{{content_type.id}}(chat, *args, **kwargs, reply_to_message: nil)
      end
    {% end %}
